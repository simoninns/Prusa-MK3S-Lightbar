// Original header from Prusa github:
...
module z_top_base()
{
    ...
    translate([0,-5,0 - 5]) cuboid([38,33,10], center=false, chamfer=2, edges=EDGES_X_ALL); // plate touching the base   
    translate([0,-5,0 - 5]) cuboid([20,45,10], center=false, chamfer=2, edges=EDGES_X_ALL); // plate touching the base   
...
module z_top_fancy()
{
    ...
    // frame side angle
    translate([-13,40+5+10.2,-3]) rotate([0,0,-45-0]) translate([0,0,-1 - 5]) cube([30,30,51 + 10]);
    translate([8,28,-3]) translate([0,0,-1 - 5]) cube([50,50,51 + 10]);       
    ...
    translate([-6,-5,-0.8 - 4.5]) rotate([0,45,0])  cube([5,50,5]);
}
...
module z_top_holes()
{
    ...
    // material saving cut
    translate([16,10,-4 - 5]) rotate([0,0,0]) cylinder(h = 50, r=8, $fn=6);  
    translate([16,28,-4 - 5]) rotate([0,0,0]) cylinder(h = 50, r=8, $fn=6);  
...
module additional_parts()
{
    difference() {
        move([88,11.5,0]) cuboid([100,33,10], chamfer=2, edges=EDGES_X_ALL);

        // Slot
        move([148-15,11.5,0]) cuboid([21,33 - 3,10 - 3], chamfer=1.5);
    }
}
